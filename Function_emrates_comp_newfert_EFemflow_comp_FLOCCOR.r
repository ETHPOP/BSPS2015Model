...
#surv.prob<-data.frame(survivorship)
#em.flow<-data.frame(emigration)
#imm.flow<-data.frame(immigration)
#inm.prob<-data.frame(inm.prob)
#outm.prob<-data.frame(outm.prob)
...
#mixingmatrix<-data.frame(mixingmatrix)
#bm
#bf
...
##FUNCTION
##Switchfromfirsttosecondifyouwanttorunmixedprojections
#ForexamplefixedemigrationinUPTAPfrom2001to2006andemratesthereafter!



projection<-function(surv.prob=allsurv2001,em.flow,imm.flow,inm.prob,outm.prob,fertility,mixingmatrix,bm,bf,initialpop,GORSlist2)
#projection2<-function(surv.prob=allsurv2001,em.rate=allemrates2001,ONSem,imm.flow,inm.prob,outm.prob,fertility,mixingmatrix,bm,bf,initialpop,GORSlist2)
...
#setemrate,heresimplehalfofbirthtooneis
...
#fininmig<-priminmig*(sum(out.migrants)/sum(priminmig))##corrcted!falschmussauchzwischengruppenunterscheiden

surv.stayers<-surv.pop-out.migrants



imm.flow<-cbind((0.5*imm.flow[1]),imm.flow[1:101],(0.5*imm.flow[102]),imm.flow[102:202])
imm.flow<-(surv.prob^0.5)*imm.flow
...
##thiscomputessumsofethnicgroupssubsestsX3inGorLAEthistheethnicgroupnumber
##maybewritefunctioncomponents
...
#return(list(Newpop,components,LAsComp))}

return(list(Newpop,components,LAsComp,fininmig,out.migrants,tot.deaths,BirthsAll,surv.em,imm.flow))}
